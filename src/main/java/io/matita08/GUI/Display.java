...
public class Display extends JFrame {
   ...
   /**
    * Constructs and adds the main GUI sections (CPU, bus labels, central memory, control panel and interfaces)
    * to the container using GridBagLayout.
    *
    * <p>Configures and positions subpanels with appropriate layout constraints:
    * - CPU panel (expands both directions)
    * - Bus labels panel
    * - Central memory (MC) displayed inside a vertically scrollable JScrollPane with a titled border
    * - Control panel with action buttons
    * - Interfaces panel</p>
    *
    * <p>The method delegates detailed population of each section to helper methods:
    * {@code createCPUComponents}, {@code createBusComponents}, {@code createMCComponents},
    * {@code createControlPanelComponents}, and {@code createInterfacesComponents}.</p>
    */
   ...
   /**
    * Creates the bus UI components for the display.
    *
    * Adds a JLabel with text "bus" to the internal `busLabels` list for use in the bus panel.
    */
   ...
   /**
    * Builds and populates the memory-central (MC) panel with address/value rows.
    *
    * <p>Creates a two-column header ("Indirizzi", "Valori") and then for each
    * memory address [0 .. Constants.MC_Size-1] adds a right-aligned address label
    * and a centered value label initialized to DoubleValue.unset(). Address labels
    * receive a right border to separate columns. Each value label is stored in
    * the MCData list for later updates. This method mutates the MC container and
    * the MCData collection.
    */
